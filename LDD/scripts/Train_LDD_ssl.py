...
def rotation_ssl_data(images, labels):
...
model_mlp = nn.Sequential(
    nn.Linear(model_d.fc.in_features//2, 4),
).to(device)
...
for epoch in range(1, main_num_steps+1):
    for ix, (index, data, attr) in enumerate(train_loader):
        ...
        data_rot, label_rot = rotation_ssl_data(data, label)
        ...
        try:
            z_r = []
            hook_fn = model_d.avgpool.register_forward_hook(concat_dummy(z_r))
            _ = model_d(data_rot)
            hook_fn.remove()
            z_r = z_r[0]
        except:
            z_r = []
            hook_fn = model_d.layer4.register_forward_hook(concat_dummy(z_r))
            _ = model_d(data_rot)
            hook_fn.remove()
            z_r = z_r[0]
        ...
        loss_rotate = criterion(z_r, label_rot.long()).mean()
        ...
        loss = loss_dis + lambda_swap * loss_swap + loss_rotate_param * loss_rotate                    # Eq.1 L_total
    ...
    schedulerb.step()
    schedulerd.step()
    print("[Loss rotation][{:.4f}]".format(round(loss_rotate.item(), 4)))
    print("[Loss dis][{:.4f}]".format(round(loss_dis.item(), 4)))
...
write_to_file('results_text/results_LDD_SSL_'+args.dataset_in.split('-')[0]+'_'+str(args.train_samples)+'_'+str(args.bias_ratio)+'.txt','[Best Test Accuracy]'+str(test_accuracy)+"[Final Epoch Test Accuracy]"+str(test_accuracy_epoch)+ '[Best Cheat Test Accuracy]'+str(test_cheat))
