...
SSL (Rotation)
'''
def rotation_ssl(model, train_loader,test_loader, epochs = 1):
...
try:
    modelssl = dic_models[args.model_in](num_classes).to(device)
except:
    modelssl = dic_models_2[args.model_in](num_classes).to(device)


modelssl = rotation_ssl(modelssl, train_loader, test_loader, 100)



try:
    model_d = dic_models[args.model_in+'_MW'](num_classes).to(device)
    model_b = dic_models[args.model_in](num_classes).to(device)
except:
    model_d = dic_models_2[args.model_in+'_MW'](num_classes).to(device)
    model_b = dic_models_2[args.model_in](num_classes).to(device)

model_d.fc = nn.Linear(model_d.fc.encoder_output_dim, 4).to(device)

model_d.load_state_dict(modelssl.state_dict())

model_d.fc = MemoryWrapLayer(model_d.fc.in_features, num_classes).to(device)
...
write_to_file('results_text/results_SSL_MW_LfF_'+args.dataset_in.split('-')[0]+'_'+str(args.train_samples)+'_'+str(args.bias_ratio)+'.txt','[Best Test Accuracy]'+str(test_accuracy)+"[Final Epoch Test Accuracy]"+str(test_accuracy_epoch)+ '[Best Cheat Test Accuracy]'+str(test_cheat))