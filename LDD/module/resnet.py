...
class ResNet_MW(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2)
        self.avgpool =  nn.AdaptiveAvgPool2d(output_size=(1, 1))
        self.fc = MemoryWrapLayer(64*2, num_classes)
        self.apply(_weights_init)
    ...
    def forward(self,x,memory_input,loss_weights , return_weights = False):
        x_out = self.forward_encoder(x)
        mem_out = self.forward_encoder(memory_input)
        out_mw = self.fc(x_out,mem_out,loss_weights,return_weights)
        return out_mw
